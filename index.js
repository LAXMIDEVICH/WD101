...
if(!validateUserDate(userData)){
    const errorMessage=document.createElement('p');
    errorMessage.textContent='Value must be 20/11/1967 or later';
...
function saveUserData(userData)
{
    ...
    loalStorage.setItem('userList',JSON.stringify(existingUserData));
}
...
function createUserDataTableRow(userData){
    ...
    row.innerHTML='<td>${userData.name}</td><td>${userData.email}</td><td>${userData.password}</td><td>${userData.dob}</td><td>${userData.terms ? 'true':'false'}</td>';
    return row;
}
function clearForm() {
